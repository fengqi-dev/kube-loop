...
func newRelaySession(connection *websocket.Conn, listeners *boundListeners, idle time.Duration, now func() time.Time) *relaySession {
	return &relaySession{
...
func (relay *relaySession) acceptTCP(ctx context.Context, binding tcpBinding, streamWG *sync.WaitGroup) error {
	for {
...
func (relay *relaySession) readUDP(ctx context.Context, index int, binding udpBinding) error {
...
func (relay *relaySession) udpAssociation(binding udpBinding, remote *net.UDPAddr, key string) uint64 {
	now := relay.now().UTC()
	relay.mu.Lock()
