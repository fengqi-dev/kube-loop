...
func bindExchangeListeners(gatewayIP string, ports []Port) (*boundListeners, error) {
	...
	for _, port := range ports {
		...
		case "tcp":
			...
			if !ok || address.Port == 0 {
				_ = listener.Close()
				return fail(errors.New("Exchange TCP listener returned an invalid address"))
			}
		...
		default:
			return fail(fmt.Errorf("unsupported Exchange protocol %q", port.Protocol))
		}
		if listenPort < 1 || listenPort > 65535 {
			return fail(errors.New("Exchange listener returned an invalid port"))
		}
