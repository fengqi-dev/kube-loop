...
func (exchangeTestResources) Capture(context.Context, controller.Principal, *servicebinding.ServiceInterceptSnapshot) error {
	return nil
}

func (exchangeTestResources) Apply(context.Context, controller.Principal, servicebinding.ServiceInterceptSnapshot, string) error {
	return nil
}
...
func TestExchangeTaskIsOwnedIdempotentAndDurablyStopped(t *testing.T) {
	...
	_, apiError = exchangeRequest(handler, controller.Principal{Subject: uuid.NewString(), DeviceID: "other"}, http.MethodGet, taskPath, nil, "")
