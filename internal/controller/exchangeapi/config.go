...
func (config *Config) normalize() error {
	...
	if address == nil || address.IsUnspecified() {
		return errors.New("Exchange Gateway IP must be a concrete IP address")
	}
	...
	if len(config.OwnerID) > 253 {
		return errors.New("Exchange owner ID is too long")
	}
	if config.Now == nil {
		config.Now = time.Now
	}
	...
	if config.CredentialCheckInterval < 10*time.Millisecond || config.CredentialCheckInterval > 30*time.Second {
		return errors.New("Exchange credential check interval must be between 10ms and 30s")
	}
	if config.TaskCheckInterval < 10*time.Millisecond || config.TaskCheckInterval > 30*time.Second {
		return errors.New("Exchange Task check interval must be between 10ms and 30s")
	}
	if config.UDPIdleTimeout < 100*time.Millisecond || config.UDPIdleTimeout > 24*time.Hour {
		return errors.New("Exchange UDP idle timeout must be between 100ms and 24h")
	}
