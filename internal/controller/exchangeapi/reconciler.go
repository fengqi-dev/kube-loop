...
func NewReconciler(
	...
	if storageBackend == nil || resources == nil {
		return nil, errors.New("Exchange recovery storage and resource mutator are required")
	}
	if logger == nil {
		logger = slog.Default()
	}
	...
	if config.Interval < 100*time.Millisecond || config.Interval > 24*time.Hour {
		return nil, errors.New("Exchange recovery interval must be between 100ms and 24h")
	}
	if config.StaleAfter < config.Interval || config.StaleAfter > 24*time.Hour {
		return nil, errors.New("Exchange stale threshold must be at least the recovery interval and at most 24h")
	}
	if config.RestoreTimeout < time.Second || config.RestoreTimeout > 5*time.Minute {
		return nil, errors.New("Exchange recovery restore timeout must be between 1s and 5m")
	}
	if config.BatchSize < 1 || config.BatchSize > 1000 {
		return nil, errors.New("Exchange recovery batch size must be between 1 and 1000")
	}
	return &Reconciler{
...
func (reconciler *Reconciler) RunOnce(ctx context.Context) (int, error) {
	if ctx == nil {
		return 0, errors.New("Exchange recovery context is required")
	}
...
func (reconciler *Reconciler) recover(ctx context.Context, task storage.Task, now time.Time) (bool, error) {
	...
	if len(snapshots) != 1 || snapshots[0].Kind != exchangeSnapshotKind {
	...
	cancel()
	if err != nil {
		reconciler.deferRecovery(task.ID, "Exchange resource restoration is pending")
		return true, err
	}
	next := remotetask.Failed
	message := "Exchange owner was lost"
...
func (reconciler *Reconciler) failWithoutSnapshot(taskID string) error {
	result := ownerResult{
		...
		Error: "Exchange rollback snapshot is missing or invalid",
	}
	encoded, _ := json.Marshal(result)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (reconciler *Reconciler) runAndLog(ctx context.Context) {
	...
	if err != nil {
		if ctx.Err() == nil {
			reconciler.logger.ErrorContext(ctx, "Exchange stale-owner recovery pass failed", "error", err)
		}
		return
	}
	if count > 0 {
		reconciler.logger.InfoContext(ctx, "Exchange stale owners reconciled", "tasks", count)
	}
}
