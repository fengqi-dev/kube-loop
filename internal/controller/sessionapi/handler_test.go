...
type auditCapture struct {
	mu      sync.Mutex
	records []controller.AuditRecord
}
...
func (staticCapabilityDiscoverer) DiscoverCapabilities(
	...
	if err != nil {
		return capability.Snapshot{}, &controller.APIError{Code: controller.CodeInternal, Cause: err}
	}
	return snapshot, nil
}
...
func (capture *auditCapture) Record(_ context.Context, record controller.AuditRecord) error {
	capture.mu.Lock()
...
func newSessionTestServer(
	t *testing.T,
	now func() time.Time,
) (*controller.Server, *storage.Store, *auditCapture, string) {
	t.Helper()
	stateStore, err := storage.Open(context.Background(), storage.Config{
		Backend: storage.BackendSQLite, SQLitePath: filepath.Join(t.TempDir(), "sessions.db"), ControllerReplicas: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := router.HandlePrefix(controller.APIPathPrefix+"/sessions", handler); err != nil {
		stateStore.Close()
		t.Fatal(err)
	}
	...
	server, err := controller.NewServer(
		...
		controller.WithAuthenticator(controller.AuthenticatorFunc(func(request *http.Request) (controller.Principal, *controller.APIError) {
			...
			return controller.Principal{Subject: subject, DeviceID: "device-1"}, nil
		})),
		controller.WithAuthorizer(policy), controller.WithAuditSink(capture), controller.WithAPIHandler(router),
	)
	if err != nil {
		stateStore.Close()
		t.Fatal(err)
