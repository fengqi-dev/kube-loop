...
func New(repositories storage.Repositories, logger *slog.Logger, config Config) (*Worker, error) {
	if repositories == nil {
		return nil, errors.New("Controller maintenance storage is required")
	}
	if logger == nil {
		logger = slog.Default()
	}
	...
	if config.Interval < 10*time.Millisecond || config.Interval > 24*time.Hour {
		return nil, errors.New("Controller maintenance interval must be between 10ms and 24h")
	}
	if config.Timeout <= 0 || config.Timeout > time.Minute {
		return nil, errors.New("Controller maintenance timeout must be between 1ns and 1m")
	}
	if config.BatchSize <= 0 || config.BatchSize > 1000 {
		return nil, errors.New("Controller maintenance batch size must be between 1 and 1000")
	}
	return &Worker{
...
func (worker *Worker) RunOnce(ctx context.Context) (Report, error) {
	if ctx == nil {
		return Report{}, errors.New("Controller maintenance context is required")
	}
...
func (worker *Worker) runAndLog(ctx context.Context) {
	...
	if err != nil {
		if ctx.Err() == nil {
			worker.logger.ErrorContext(ctx, "Controller maintenance pass failed", "error", err)
		}
		return
	}
	if report.Total() > 0 {
