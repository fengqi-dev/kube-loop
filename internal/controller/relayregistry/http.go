...
func NewHTTPHandler(registry *Registry, authenticator Authenticator) (*HTTPHandler, error) {
	...
	router.Post(InternalPathPrefix+"/register", handler.register)
	router.Put(InternalPathPrefix+"/heartbeat", handler.heartbeat)
...
func (handler *HTTPHandler) heartbeat(writer http.ResponseWriter, request *http.Request) {
	identity, err := handler.authenticator.Authenticate(request)
	if err != nil {
		writeInternalError(writer, http.StatusUnauthorized, "unauthenticated", "Relay workload identity is invalid")
		return
