...
func NewTokenReviewAuthenticator(client kubernetes.Interface, config TokenReviewConfig) (*TokenReviewAuthenticator, error) {
	probe := relaycontrol.PeerIdentity{
		TrustDomain: strings.TrimSpace(config.TrustDomain), Namespace: strings.TrimSpace(config.Namespace),
		ServiceAccount: strings.TrimSpace(config.ServiceAccount), PodUID: "probe",
	}
	...
	if client == nil || config.Audience == "" || len(config.Audience) > 256 || probe.Validate() != nil {
		return nil, errors.New("Relay TokenReview identity configuration is invalid")
	}
	return &TokenReviewAuthenticator{client: client, config: config}, nil
}

func (authenticator *TokenReviewAuthenticator) Authenticate(request *http.Request) (relaycontrol.PeerIdentity, error) {
	if authenticator == nil || request == nil {
		return relaycontrol.PeerIdentity{}, errors.New("Relay TokenReview authenticator is unavailable")
	}
	headers := request.Header.Values("Authorization")
	if len(headers) != 1 {
		return relaycontrol.PeerIdentity{}, errors.New("one Relay workload bearer token is required")
	}
	parts := strings.Fields(headers[0])
	if len(parts) != 2 || !strings.EqualFold(parts[0], "Bearer") || len(parts[1]) > 16<<10 {
		return relaycontrol.PeerIdentity{}, errors.New("Relay workload bearer token is invalid")
	}
	review, err := authenticator.client.AuthenticationV1().TokenReviews().Create(
		request.Context(),
	...
	if err != nil || review == nil || !review.Status.Authenticated ||
		!containsString(review.Status.Audiences, authenticator.config.Audience) {
		return relaycontrol.PeerIdentity{}, errors.New("Relay workload token was not authenticated")
	}
	...
	if review.Status.User.Username != wantUsername {
		return relaycontrol.PeerIdentity{}, errors.New("Relay workload ServiceAccount is not allowed")
	}
	podUIDs := review.Status.User.Extra[podUIDExtra]
	if len(podUIDs) != 1 || strings.TrimSpace(podUIDs[0]) == "" {
		return relaycontrol.PeerIdentity{}, errors.New("Relay workload token is not bound to one Pod")
	}
