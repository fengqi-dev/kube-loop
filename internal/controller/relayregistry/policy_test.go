...
func TestKubernetesTopologyResolverBindsUIDServiceAccountAndNode(t *testing.T) {
	...
	controllerPod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "controller", Namespace: "kubeloop"},
		Spec: corev1.PodSpec{NodeName: "node-a"}}
	if _, err := client.CoreV1().Pods("kubeloop").Create(context.Background(), controllerPod, metav1.CreateOptions{}); err != nil {
		t.Fatal(err)
	}
	controllerTopology, err := KubernetesPodTopology(context.Background(), client, "kubeloop", "controller")
	if err != nil || controllerTopology[TopologyRegion] != "cn" {
		t.Fatalf("Controller topology = %#v, %v", controllerTopology, err)
	}
}
