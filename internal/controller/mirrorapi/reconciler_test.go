...
func TestMirrorReconcilerClaimsStaleOwnerOnceAndRestores(t *testing.T) {
	stateStore, principal, active := mirrorStreamStore(t)
	now := time.Now().UTC()
	task := staleMirrorTask(t, stateStore, principal.Subject, active.ID, now.Add(-time.Minute), true)
...
func TestMirrorReconcilerRetainsSnapshotAndRetriesRestore(t *testing.T) {
	stateStore, principal, active := mirrorStreamStore(t)
	clock := time.Now().UTC()
	task := staleMirrorTask(t, stateStore, principal.Subject, active.ID, clock.Add(-time.Minute), true)
