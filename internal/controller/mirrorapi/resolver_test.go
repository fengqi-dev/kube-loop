...
type mirrorTestProvider struct{ client kubernetes.Interface }

func (provider mirrorTestProvider) ClientFor(authorization.Subject) (kubernetes.Interface, error) {
...
func TestKubernetesResolverRequiresEveryRequestedPortToBeAuthoritative(t *testing.T) {
	...
	resolver, err := NewKubernetesServiceResolver(mirrorTestProvider{client: client})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || len(partial.Ports) != 1 || partial.Ports[0].Name != "http" {
		t.Fatalf("selected Mirror port=%#v err=%v", partial, err)
	}
	if _, err := resolver.ResolveService(
		context.Background(), controller.Principal{Subject: "user"}, "development", "api",
		[]Port{{ServicePort: 53, Protocol: "tcp"}, {ServicePort: 80, Protocol: "tcp"}},
	); err == nil {
		t.Fatal("mismatched Mirror protocol was accepted")
	}
}
