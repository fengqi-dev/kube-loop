...
func (handler *Handler) create(writer http.ResponseWriter, request *http.Request, principal controller.Principal, session sessionapi.ActiveSession) *controller.APIError {
	...
	writer.Header().Set("Location", fmt.Sprintf("%s/sessions/%s/mirrors/%s/stream?namespace=%s", controller.APIPathPrefix, session.ID, task.ID, session.Namespace))
	if !created {
		writer.Header().Set("Idempotent-Replayed", "true")
	}
...
func routeParts(path string) ([]string, bool) {
	...
	if len(parts) < 2 || parts[1] != "mirrors" {
		return nil, false
	}
...
func targetError(err error) *controller.APIError {
	...
	case apierrors.IsForbidden(err):
		return &controller.APIError{Code: controller.CodeForbidden, Message: "Kubernetes Mirror access is not permitted", Cause: err}
...
func storageError(err error) *controller.APIError {
	...
	case errors.Is(err, storage.ErrConflict), errors.Is(err, storage.ErrIdempotencyMismatch):
		return &controller.APIError{Code: controller.CodeConflict, Message: "Mirror Task conflicts with existing state", Cause: err}
	default:
		return internalError(err)
	}
}
...
func internalError(err error) *controller.APIError {
	return &controller.APIError{Code: controller.CodeInternal, Message: "Mirror operation failed", Cause: err}
}
