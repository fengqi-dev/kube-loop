...
func (transferExecutor) UploadOffset(context.Context, controller.Principal, string, fileapi.Spec) (uint64, error) {
	return 0, nil
}
...
func (executor *offsetExecutor) UploadOffset(context.Context, controller.Principal, string, fileapi.Spec) (uint64, error) {
	executor.calls++
	return executor.offset, nil
}

func (transferExecutor) Upload(
	context.Context, controller.Principal, string, string, fileapi.Spec, io.Reader,
...
func (*streamExecutor) UploadOffset(context.Context, controller.Principal, string, fileapi.Spec) (uint64, error) {
	return 0, nil
}
...
func (transferExecutor) Download(
	context.Context, controller.Principal, string, string, fileapi.Spec,
...
func TestFileTransferTaskIsValidatedOwnedAndIdempotent(t *testing.T) {
	...
	handler, err := fileapi.New(stateStore, sessionValidator{principalID: principalID, session: sessionapi.ActiveSession{
	...
	if err := router.Handle(http.MethodPost, "/api/v2/sessions/{sessionID}/file-transfers", handler); err != nil {
		t.Fatal(err)
	}
	if err := router.Handle(http.MethodGet, "/api/v2/sessions/{sessionID}/file-transfers/{taskID}", handler); err != nil {
		t.Fatal(err)
	}
	...
	document := createTask(t, httpServer.URL, principalID, sessionID, "file-1", body, http.StatusCreated)
	...
	replayed := createTask(t, httpServer.URL, principalID, sessionID, "file-1", body, http.StatusOK)
	...
	resolver.mu.Unlock()
	getRequest, _ := http.NewRequest(http.MethodGet, httpServer.URL+"/api/v2/sessions/"+sessionID+"/file-transfers/"+document.ID+"?namespace=development", nil)
	...
	conflict := createTask(t, httpServer.URL, principalID, sessionID, "file-1", strings.Replace(body, `"size":7`, `"size":8`, 1), http.StatusConflict)
	if conflict.ID != "" {
...
func TestFileTransferRejectsTraversalRootAndUntrustedDownloadMetadata(t *testing.T) {
	...
	for _, spec := range []fileapi.Spec{
		...
		request := httptest.NewRequest(http.MethodPost, "/api/v2/sessions/"+sessionID+"/file-transfers?namespace=development", bytes.NewReader(raw))
		request.Header.Set("Content-Type", "application/json")
		request.Header.Set("Idempotency-Key", uuid.NewString())
		response := httptest.NewRecorder()
		apiError := handler.ServeAPI(response, request, controller.Principal{Subject: principalID, DeviceID: "device"})
...
func TestFileTransferTaskUsesControllerAuthoritativeResumeOffset(t *testing.T) {
	now := time.Now().UTC()
	stateStore, principalID, sessionID, expiresAt := createStore(t, now)
	defer stateStore.Close()
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v2/sessions/"+sessionID+"/file-transfers?namespace=development", strings.NewReader(body))
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("Idempotency-Key", "resume-offset")
	response := httptest.NewRecorder()
	if apiError := handler.ServeAPI(response, request, controller.Principal{Subject: principalID}); apiError != nil {
		t.Fatal(apiError)
	}
...
func TestFileTransferWebSocketUploadDownloadAndSingleClaim(t *testing.T) {
	...
	uploadTask := createTask(t, httpServer.URL, principalID, sessionID, "upload-stream", string(uploadBody), http.StatusCreated)
	uploadURL := fileStreamURL(httpServer.URL, sessionID, uploadTask.ID)
	uploadConnection := dialFileStream(t, uploadURL, principalID)
	...
	_, replayResponse, replayErr := websocket.Dial(context.Background(), uploadURL, &websocket.DialOptions{
		HTTPHeader: http.Header{"X-Principal": {principalID}},
	})
	...
	downloadTask := createTask(t, httpServer.URL, principalID, sessionID, "download-stream", string(downloadBody), http.StatusCreated)
	downloadConnection := dialFileStream(t, fileStreamURL(httpServer.URL, sessionID, downloadTask.ID), principalID)
...
func dialFileStream(t *testing.T, streamURL, principalID string) *websocket.Conn {
	t.Helper()
...
func fileStreamURL(baseURL, sessionID, taskID string) string {
	return "ws" + strings.TrimPrefix(baseURL, "http") + "/api/v2/sessions/" + sessionID +
...
func createStore(t *testing.T, now time.Time) (*storage.Store, string, string, time.Time) {
	...
	stateStore, err := storage.Open(context.Background(), storage.Config{
		Backend: storage.BackendSQLite, SQLitePath: filepath.Join(t.TempDir(), "files.db"), ControllerReplicas: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := stateStore.Principals().Upsert(context.Background(), storage.Principal{
		ID: principalID, Provider: "test", ExternalID: "files-user", CreatedAt: now, UpdatedAt: now,
	}); err != nil {
		t.Fatal(err)
	}
...
func createTask(
	t *testing.T,
	baseURL, principalID, sessionID, key, body string,
	wantStatus int,
) fileapi.Document {
	t.Helper()
	request, _ := http.NewRequest(http.MethodPost, baseURL+"/api/v2/sessions/"+sessionID+"/file-transfers?namespace=development", strings.NewReader(body))
