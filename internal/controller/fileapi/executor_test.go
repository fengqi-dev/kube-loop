...
func TestKubernetesTransferExecutorUsesOnlyGeneratedQuotedCommands(t *testing.T) {
	...
	outcome, err := executor.Upload(context.Background(), controller.Principal{Subject: "user"}, "development", taskID, spec, bytes.NewReader(contents))
	if err != nil {
		t.Fatal(err)
	}
...
func TestKubernetesTransferExecutorRejectsParentSymlinkOutsideAllowedRoot(t *testing.T) {
	...
	if _, err := executor.Upload(context.Background(), controller.Principal{Subject: "user"}, "development", "task", spec, bytes.NewReader(contents)); err == nil {
...
func TestKubernetesTransferExecutorNegotiatesAndResumesStablePartialUpload(t *testing.T) {
	...
	offset, err := executor.UploadOffset(context.Background(), controller.Principal{Subject: "user"}, "development", spec)
	if err != nil || offset != 7 {
		t.Fatalf("offset = %d err = %v", offset, err)
	}
	spec.Offset = offset
	outcome, err := executor.Upload(context.Background(), controller.Principal{Subject: "user"}, "development", "new-task", spec, bytes.NewReader(contents[offset:]))
