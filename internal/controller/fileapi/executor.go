...
type TransferExecutor interface {
	Upload(context.Context, controller.Principal, string, string, Spec, io.Reader) (Outcome, error)
	Download(context.Context, controller.Principal, string, string, Spec, func(DownloadMetadata) error, io.Writer) (Outcome, error)
	UploadOffset(context.Context, controller.Principal, string, Spec) (uint64, error)
}

type PodExecutor interface {
	Exec(context.Context, controller.Principal, string, execapi.Spec, execapi.Streams) error
}
...
// PhysicalPathGuard emits a Controller-owned shell prefix that rejects a
