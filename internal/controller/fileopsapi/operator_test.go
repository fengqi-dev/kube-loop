...
func (localPodExecutor) Exec(ctx context.Context, _ controller.Principal, _ string, spec execapi.Spec, streams execapi.Streams) error {
...
func TestKubernetesOperatorRejectsParentAndTargetSymlinkEscape(t *testing.T) {
	...
	for _, target := range []string{filepath.Join(root, "escaped", "created"), filepath.Join(root, "target-link")} {
		err := operator.Mutate(context.Background(), controller.Principal{Subject: "user"}, "development", Spec{
