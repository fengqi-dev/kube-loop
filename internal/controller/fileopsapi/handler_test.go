...
func (sessions testSessions) RequireActive(_ context.Context, principal controller.Principal, namespace, sessionID string) (sessionapi.ActiveSession, *controller.APIError) {
...
func (targets *testTargets) ResolveContainer(_ context.Context, _ controller.Principal, namespace, pod, container string) (string, error) {
	targets.mu.Lock()
	targets.calls++
	targets.mu.Unlock()
...
func (operator *testOperator) List(_ context.Context, _ controller.Principal, _ string, spec fileopsapi.Spec) ([]fileopsapi.Entry, error) {
	operator.mu.Lock()
	operator.listCalls++
	operator.mu.Unlock()
...
func (operator *testOperator) Mutate(context.Context, controller.Principal, string, fileopsapi.Spec) error {
	operator.mu.Lock()
...
func TestListAndMutationTaskAreValidatedOwnedAndIdempotent(t *testing.T) {
	...
	if apiError := handler.ServeAPI(listResponse, listRequest, controller.Principal{Subject: principalID}); apiError != nil {
		t.Fatal(apiError)
	}
	...
	if apiError := handler.ServeAPI(getResponse, getRequest, controller.Principal{Subject: principalID}); apiError != nil {
		t.Fatal(apiError)
	}
...
func TestInvalidAndRootMutationNeverReachKubernetes(t *testing.T) {
	...
	for _, test := range []struct{ action, body string }{
		...
		apiError := handler.ServeAPI(response, request(t, http.MethodPost, sessionID, test.action, test.body, uuid.NewString()), controller.Principal{Subject: principalID})
...
func mutate(t *testing.T, handler *fileopsapi.Handler, principalID, sessionID, action, body, key string, status int) fileopsapi.Document {
	t.Helper()
	response := httptest.NewRecorder()
	apiError := handler.ServeAPI(response, request(t, http.MethodPost, sessionID, action, body, key), controller.Principal{Subject: principalID})
	if apiError != nil {
		t.Fatal(apiError)
	}
...
func request(t *testing.T, method, sessionID, suffix, body, key string) *http.Request {
	...
	request := httptest.NewRequest(method, "/api/v2/sessions/"+sessionID+"/pod-files/"+suffix+"?namespace=development", content)
	if body != "" {
...
func createStore(t *testing.T, now time.Time) (*storage.Store, string, string, time.Time) {
	...
	stateStore, err := storage.Open(context.Background(), storage.Config{
		Backend: storage.BackendSQLite, SQLitePath: filepath.Join(t.TempDir(), "file-operations.db"), ControllerReplicas: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
