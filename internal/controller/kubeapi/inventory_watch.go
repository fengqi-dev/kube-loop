...
func (handler *Handler) watchInventory(
	...
	if len(request.URL.Query()) != 1 || len(request.URL.Query()["watch"]) != 1 || request.URL.Query().Get("watch") != "true" {
		return &controller.APIError{Code: controller.CodeInvalidArgument, Field: "watch", Message: "watch=true is required"}
	}
	...
	if err != nil {
		return &controller.APIError{Code: controller.CodeUnavailable, Message: "Inventory Watch is unavailable", Cause: err}
	}
	defer unsubscribe()
