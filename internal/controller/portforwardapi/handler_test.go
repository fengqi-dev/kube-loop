...
type captureResolver struct {
	mu    sync.Mutex
	calls []portforwardapi.Spec
}

type bindingManager struct{}

func (bindingManager) Activate(context.Context, sessionapi.ActiveSession, string, portforwardapi.Spec) (bool, error) {
	return true, nil
}
...
func (resolver *captureResolver) Resolve(
	...
	_ string,
	spec portforwardapi.Spec,
) (portforwardapi.Target, error) {
	resolver.mu.Lock()
	resolver.calls = append(resolver.calls, spec)
	resolver.mu.Unlock()
	return portforwardapi.Target{Host: "10.96.0.20", Port: spec.RemotePort}, nil
}
...
func TestPortForwardTaskLifecycleIsOwnedIdempotentAndPolicyMapped(t *testing.T) {
	...
	stateStore, err := storage.Open(context.Background(), storage.Config{
		Backend: storage.BackendSQLite, SQLitePath: filepath.Join(t.TempDir(), "controller.db"), ControllerReplicas: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	handler, err := portforwardapi.New(stateStore, ownedSessionValidator{
		principalID: principalID,
		session: sessionapi.ActiveSession{
			ID: sessionID, Namespace: "development", Generation: 1, ExpiresAt: expiresAt, NetworkSpecHash: specHash,
		},
	}, resolver, bindingManager{}, portforwardapi.Config{Now: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	...
	legacySpec := portforwardapi.Spec{Kind: "service", Name: "api", Protocol: "tcp", RemotePort: 8443}
	legacySpecJSON, _ := json.Marshal(legacySpec)
	legacyTargetJSON, _ := json.Marshal(portforwardapi.Target{Host: "10.96.0.20", Port: 8443})
	legacyEnvelope, _ := json.Marshal(struct {
		SessionID string              `json:"sessionId"`
		Namespace string              `json:"namespace"`
		Spec      portforwardapi.Spec `json:"spec"`
	}{SessionID: sessionID, Namespace: "development", Spec: legacySpec})
	legacyTask := storage.Task{
		ID: uuid.NewString(), PrincipalID: principalID, SessionID: sessionID, Type: portforwardapi.TaskType,
	...
	if _, created, err := stateStore.Idempotency().Reserve(context.Background(), storage.IdempotencyRecord{
		Scope: taskapi.Scope(portforwardapi.TaskType, principalID), Key: "legacy-pf",
		RequestHash: taskapi.Hash(legacyEnvelope), ResourceType: portforwardapi.TaskType, ResourceID: legacyTask.ID,
