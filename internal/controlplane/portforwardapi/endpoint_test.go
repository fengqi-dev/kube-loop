...
func TestPortForwardTaskLifecycleIsOwnedIdempotentAndPolicyMapped(
	t *testing.T,
) {
	now := time.Date(2026, 8, 10, 8, 0, 0, 0, time.UTC)
	stateStore, err := storage.Open(context.Background(), storage.Config{
		Backend:              storage.BackendSQLite,
		SQLitePath:           filepath.Join(t.TempDir(), "controlplane.db"),
	...
	service, err := portforwardservice.New(
		stateStore,
		resolver,
		bindingManager{},
		portforwardservice.Config{Now: func() time.Time { return now }},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	server, err := controlplane.NewServer(
		...
		controlplane.WithAPIRoutes(
			controlplane.APIRoutes{
				PortForwards: portforwardapi.NewRoutes(service, sessions).
					Endpoints(),
			},
		),
	)
	...
	if document.SessionID != sessionID || document.State != "running" ||
		document.DialAddress != "10.96.0.20:8443" {
		t.Fatalf("created task = %#v", document)
	}
	replayed := taskRequest(
		t,
		server,
		http.MethodPost,
		path,
	...
	if replayed.Code != http.StatusOK ||
		...
		decodeTaskDocument(t, replayed).ID != document.ID {
		t.Fatalf(
			"replay status = %d headers = %#v body = %s",
			replayed.Code,
			replayed.Header(),
	...
	if len(resolver.calls) != 1 {
		t.Fatalf(
			"idempotent replay resolved Kubernetes target %d times",
			len(resolver.calls),
		)
	}
	resolver.mu.Unlock()
	mismatch := taskRequest(
		t,
		server,
		http.MethodPost,
		path,
		...
		"pf-1",
		[]byte(
			`{"kind":"service","name":"api","protocol":"tcp","remotePort":9443}`,
		),
	)
	if mismatch.Code != http.StatusConflict {
		t.Fatalf(
			"mismatch status = %d body = %s",
			mismatch.Code,
	...
	if listed.Code != http.StatusOK ||
		json.Unmarshal(listed.Body.Bytes(), &list) != nil ||
		len(list.Items) != 1 {
		t.Fatalf(
...
func decodeTaskDocument(
	t *testing.T,
	response *httptest.ResponseRecorder,
) portforwardapi.Document {
	t.Helper()
