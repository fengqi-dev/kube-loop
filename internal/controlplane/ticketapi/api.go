...
func (routes *Routes) issue(ctx *echo.Context, identity controlplaneapi.Identity) *controlplaneapi.Error {
	request := ctx.Request()
	...
	session, apiError := routes.sessions.RequireActive(request.Context(), identity, namespace, sessionID)
	if apiError != nil {
		return apiError
	}
	...
	ticket, err := routes.service.Issue(request.Context(), ticketservice.IssueInput{
		IdentityID: identity.Subject, Groups: append([]string(nil), identity.Groups...), DeviceID: identity.DeviceID,
