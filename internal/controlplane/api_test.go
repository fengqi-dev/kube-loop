...
func TestAPIDefaultsToAuthenticationRequired(t *testing.T) {
	...
	if document.Error.Code != controlplaneapi.CodeUnauthenticated || document.Error.RequestID == "" {
...
func TestAPIBodyLimitAndEchoBinding(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "too large", contentType: "application/json", body: `{"name":"0123456789"}`, status: http.StatusBadRequest, message: "request body exceeds the size limit"},
		{name: "unknown field", contentType: "application/json", body: `{"other":1}`, status: http.StatusOK},
		{name: "multiple documents", contentType: "application/json", body: `{}` + "\n" + `{}`, status: http.StatusBadRequest, message: "request binding failed"},
		{name: "wrong content type", contentType: "text/plain", body: `{}`, status: http.StatusBadRequest, message: "request binding failed"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			document := decodeAPIError(t, response)
			if document.Error.Code != controlplaneapi.CodeInvalidArgument || document.Error.Message != test.message {
				t.Fatalf("unexpected error: %#v", document.Error)
			}
		})
	}
