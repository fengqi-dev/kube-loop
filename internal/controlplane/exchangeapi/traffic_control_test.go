...
func TestTrafficSessionAcceptsAuthenticatedOlderGeneration(t *testing.T) {
	...
	handler := &Service{sessions: exchangeTestSessions{session: sessionapi.ActiveSession{
	...
	tests := []struct {
		...
		{name: "older authenticated generation", streamGeneration: currentGeneration - 1},
		{name: "future generation", streamGeneration: currentGeneration + 1, wantConflict: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, _, apiError := handler.trafficSession(context.Background(), trafficcontrol.Identity{
			...
			if test.wantConflict {
				if apiError == nil || apiError.Code != controlplaneapi.CodeConflict {
					t.Fatalf("trafficSession() error = %#v, want conflict", apiError)
				}
				return
			}
			if apiError != nil {
