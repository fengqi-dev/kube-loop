...
func (handler *Service) create(ctx *echo.Context, identity controlplaneapi.Identity, session sessionapi.ActiveSession) *controlplaneapi.Error {
	...
	ctx.Response().Header().Set("Location", fmt.Sprintf("%s/sessions/%s/exchanges/%s?namespace=%s", controlplane.SessionAPIPathPrefix, session.ID, task.ID, session.Namespace))
	if !created {
		ctx.Response().Header().Set("Idempotent-Replayed", "true")
	}
...
func targetError(err error) *controlplaneapi.Error {
	...
	case apierrors.IsForbidden(err):
		return &controlplaneapi.Error{Code: controlplaneapi.CodeForbidden, Message: "Kubernetes Exchange access is not permitted", Cause: err}
...
func storageError(err error) *controlplaneapi.Error {
	...
	case errors.Is(err, storage.ErrConflict), errors.Is(err, storage.ErrIdempotencyMismatch):
		return &controlplaneapi.Error{Code: controlplaneapi.CodeConflict, Message: "Exchange Task conflicts with existing state", Cause: err}
	default:
		return internalError(err)
	}
}
...
func internalError(err error) *controlplaneapi.Error {
	return &controlplaneapi.Error{Code: controlplaneapi.CodeInternal, Message: "Exchange operation failed", Cause: err}
}
