...
func (handler *Service) delete(
	...
	if err := deletePreviewBinding(ctx.Request().Context(), handler.resources, session.Namespace, task.ID); err != nil {
		return internalError(err)
	}
	if task.State != remotetask.Stopped {
		now := handler.now().UTC()
...
func deletePreviewBinding(
	ctx context.Context, resources ResourceManager, namespace, taskID string,
) error {
