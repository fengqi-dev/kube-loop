...
func postgresqlSchemaStatement(statement string) string {
	...
	for column := range postgresqlBooleanColumns {
		statement = strings.ReplaceAll(
			statement,
			column+" INTEGER NOT NULL DEFAULT 1 CHECK ("+column+" IN (0, 1))",
			column+" BOOLEAN NOT NULL DEFAULT TRUE",
		)
		statement = strings.ReplaceAll(
			statement,
			column+" INTEGER NOT NULL DEFAULT 0 CHECK ("+column+" IN (0, 1))",
			column+" BOOLEAN NOT NULL DEFAULT FALSE",
		)
		statement = strings.ReplaceAll(
			statement,
			column+" INTEGER NOT NULL CHECK ("+column+" IN (0, 1))",
