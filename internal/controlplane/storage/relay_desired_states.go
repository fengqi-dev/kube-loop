...
func (repository *relayDesiredStateRepository) Get(ctx context.Context, relayID string) (RelayDesiredState, error) {
	...
	if err != nil {
		return RelayDesiredState{}, databaseError("read Relay desired state", err)
	}
	return value, nil
}
...
func normalizeRelayDesiredState(value *RelayDesiredState) error {
	...
	value.UpdatedAuthenticationType = strings.TrimSpace(value.UpdatedAuthenticationType)
	value.Reason = strings.TrimSpace(value.Reason)
	if (value.DesiredState != "ready" && value.DesiredState != "draining") || value.Version == 0 ||
		value.UpdatedBy == "" || value.Reason == "" || value.UpdatedAt.IsZero() {
...
func scanRelayDesiredState(row rowScanner) (RelayDesiredState, error) {
	...
	var err error
	value.UpdatedAt, err = parseTime(updatedAt, "Relay desired state update time")
	return value, err
}
