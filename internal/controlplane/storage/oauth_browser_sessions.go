...
func (repository *oauthBrowserSessionRepository) Create(
	ctx context.Context,
	session OAuthBrowserSession,
) error {
	if len(session.IDHash) != sha256Size || session.IdentityID == "" || session.ProviderID == "" ||
		session.AuthTime.IsZero() || session.CreatedAt.IsZero() ||
	...
	_, err := repository.executor.ExecContext(
		ctx,
		repository.bind(`INSERT INTO oauth_browser_sessions(
...
func (repository *oauthBrowserSessionRepository) Get(
	ctx context.Context,
	hash []byte,
	now time.Time,
) (OAuthBrowserSession, error) {
	...
	err := repository.executor.QueryRowContext(ctx, repository.bind(`SELECT id_hash, identity_id, provider_id,
		...
		WHERE id_hash = ? AND revoked_at IS NULL AND expires_at > ?`), hash, formatTime(now)).
		Scan(
	...
	if err != nil {
		return OAuthBrowserSession{}, databaseError(
			"read OAuth browser session",
			err,
		)
	}
	value.AuthTime, err = parseTime(
		authTime,
		"OAuth browser session authentication time",
	)
	if err == nil {
		value.CreatedAt, err = parseTime(
			createdAt,
			"OAuth browser session creation time",
		)
	}
	if err == nil {
		value.ExpiresAt, err = parseTime(
			expiresAt,
			"OAuth browser session expiry",
		)
	}
	if err == nil && revokedAt.Valid {
		...
		parsed, err = parseTime(
			revokedAt.String,
			"OAuth browser session revocation time",
		)
...
func (repository *oauthBrowserSessionRepository) Revoke(
	ctx context.Context,
	hash []byte,
	now time.Time,
) error {
	...
	result, err := repository.executor.ExecContext(
		ctx,
		repository.bind(`UPDATE oauth_browser_sessions
		SET revoked_at = COALESCE(revoked_at, ?) WHERE id_hash = ?`),
		formatTime(now),
		hash,
	)
	if err != nil {
...
func (repository *oauthBrowserSessionRepository) DeleteExpired(
	ctx context.Context,
	now time.Time,
	limit int,
) (int64, error) {
	...
	result, err := repository.executor.ExecContext(
		ctx,
		repository.bind(
			`DELETE FROM oauth_browser_sessions WHERE expires_at <= ?`,
		),
		formatTime(now),
	)
	if err != nil {
