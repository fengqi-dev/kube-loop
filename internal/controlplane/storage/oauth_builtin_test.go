...
func TestEnsureBuiltinOAuthClientsUsesDesktopProtocolCallback(t *testing.T) {
	...
	if err := EnsureBuiltinOAuthClients(t.Context(), store.OAuthClients(), "https://gateway.example/callback"); err != nil {
		t.Fatal(err)
	}
	...
	if len(desktop.RedirectURIs) != 1 || desktop.RedirectURIs[0] != DesktopOAuthRedirectURI {
