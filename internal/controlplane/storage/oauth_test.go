...
func TestOAuthSessionRepositoryListsGrantsWithoutTokenMaterial(t *testing.T) {
	...
	if _, err := store.Identities().Create(ctx, Identity{
		ID: identityID, Type: "human", DisplayName: "Grant Owner", Status: "active",
	...
	for index, requestID := range requestIDs {
		if err := store.OAuthSessions().Create(ctx, OAuthSession{
			Kind: "refresh_token", SignatureHash: bytes.Repeat([]byte{byte(index + 1)}, 32), RequestID: requestID,
			IdentityID: identityID, ClientID: "client-a", DeviceID: "device-a",
			RequestJSON: json.RawMessage(`{"granted_scopes":["openid","kubeloop.api"],"secret":"must-not-leak"}`),
			Status:      "active", CreatedAt: now.Add(time.Duration(index) * time.Minute), ExpiresAt: now.Add(2 * time.Hour),
		}); err != nil {
			t.Fatal(err)
		}
	}
	items, err := store.OAuthSessions().ListGrants(ctx, OAuthGrantListFilter{
		IdentityID: identityID, ClientID: "client-a", Status: "active", Limit: 2,
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(items) != 2 || items[0].RequestID != requestIDs[2] || items[1].RequestID != requestIDs[1] {
		t.Fatalf("first page=%+v", items)
	}
	if len(items[0].Scopes) != 2 || items[0].Scopes[0] != "kubeloop.api" || items[0].Scopes[1] != "openid" {
