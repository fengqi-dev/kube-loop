...
func OwnerForTask(
	...
	if repositories == nil {
		return Owner{}, errors.New("traffic binding storage is required")
	}
	...
	if task.Type != expectedType {
		return Owner{}, fmt.Errorf(
			"task %s has type %q instead of %q",
			task.ID,
			task.Type,
			expectedType,
		)
	...
	if session.ID != task.SessionID || session.Namespace != expectedNamespace {
		return Owner{}, errors.New(
			"traffic binding Task and Session ownership do not match",
		)
	}
	if session.Generation == 0 || session.Generation > math.MaxInt64 {
		return Owner{}, errors.New(
			"traffic binding Session generation is invalid",
		)
	}
	return Owner{
