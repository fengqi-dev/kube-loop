...
func TestResolveSnapshotBackendsFromEndpointSlices(t *testing.T) {
		...
		EndpointSlices: []discoveryv1.EndpointSlice{
			{
				Endpoints: []discoveryv1.Endpoint{
					{Addresses: []string{"10.244.0.12"}, Conditions: discoveryv1.EndpointConditions{Ready: &ready}},
					...
					{Addresses: []string{"10.244.0.13"}, Conditions: discoveryv1.EndpointConditions{Ready: &ready, Terminating: &terminating}},
				},
	...
	if len(sets) != 2 || sets[0].ServicePort != 53 || sets[0].Targets[0].Port != 5353 ||
		len(sets[1].Targets) != 2 || sets[1].Targets[0].Address != "10.244.0.10" || sets[1].Targets[0].Port != 8080 {
...
func TestResolveSnapshotBackendsSupportsUnnamedTargetPortAndLegacyEndpoints(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil || len(sets) != 1 || len(sets[0].Targets) != 1 || sets[0].Targets[0].Port < 8080 {
...
func TestResolveSnapshotBackendsRejectsMissingReadyTarget(t *testing.T) {
	...
	_, err := ResolveSnapshotBackends(ServiceInterceptSnapshot{
		...
		EndpointSlices: []discoveryv1.EndpointSlice{{
			Endpoints: []discoveryv1.Endpoint{{
				Addresses: []string{"10.244.0.10"}, Conditions: discoveryv1.EndpointConditions{Ready: &notReady},
			}},
			Ports: []discoveryv1.EndpointPort{{
				Name: new("http"), Protocol: new(corev1.ProtocolTCP), Port: new(int32(8080)),
			}},
