...
func TestReadOnlyKubernetesRoutesUseIdentityAndStableDocuments(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			...
			if response.Header().Get("Cache-Control") != "no-store" || response.Header().Get(echo.HeaderXRequestID) == "" {
...
func newServerWithPolicy(t *testing.T, upstreamURL string, policy authorization.Authorizer) *controlplane.Server {
	t.Helper()
	provider, err := controlplanekubernetes.NewForRESTConfig(&rest.Config{Host: upstreamURL}, controlplanekubernetes.Config{
