...
func TestRejectionsNameThisContract(t *testing.T) {
	if _, err := Encode(Frame{Type: Data, StreamID: 1, Payload: make([]byte, MaximumData+1)}); err == nil ||
		...
		t.Fatalf("oversize rejection = %v, want one naming mirror", err)
	}
	if _, err := Decode([]byte{Ready}); err == nil || !strings.Contains(err.Error(), "mirror") {
		t.Fatalf("truncated rejection = %v, want one naming mirror", err)
	}
}
