...
var (
	...
	consoleCard         = lipgloss.NewStyle().Border(lipgloss.RoundedBorder()).BorderForeground(consoleBorder).Background(consolePanel).Padding(1, 2)
	consoleDetail       = consoleCard.Copy().BorderForeground(consolePrimary)
	consoleOverlayBox   = consoleCard.Copy().BorderForeground(consoleAmber).Padding(1, 3)
	consoleSelected     = lipgloss.NewStyle().Foreground(consoleCanvas).Background(consoleTeal).Bold(true)
	consoleButton       = lipgloss.NewStyle().Foreground(consoleCanvas).Background(consolePrimary).Bold(true).Padding(0, 1)
	consoleDangerButton = consoleButton.Copy().Background(consoleDanger)
	...
	consoleCmdPrompt    = lipgloss.NewStyle().Foreground(consoleCanvas).Background(consoleAmber).Bold(true).Padding(0, 1)
	consoleFilterPrompt = lipgloss.NewStyle().Foreground(consoleCanvas).Background(consolePrimary).Bold(true).Padding(0, 1)
...
func (m *Model) updateConsole(message tea.Msg) (tea.Cmd, bool) {
	...
	case ":":
		...
		return nil, true
	case "/":
		if m.mode == viewMain && (m.activeTab == tabWorkloads || m.activeTab == tabServices || m.activeTab == tabTasks) {
	...
	case "enter", "x", "c":
		...
		if m.mode == viewMain && m.activeTab == tabTasks && key.String() == "enter" && m.consoleItemCount() > 0 {
			if row, ok := m.selectedConsoleTask(); ok {
				m.console.pendingTask = row.index
			}
			m.console.overlay = overlayConfirmTask
...
func (m *Model) runConsoleCommand(command string) tea.Cmd {
	...
	case "c", "conn", "connection":
		return m.gotoConsoleTab(tabConnection)
	case "w", "po", "pod", "pods", "workloads":
...
func (m Model) viewConsole() string {
	...
	var body string
	if m.width >= consoleWideWidth {
		body = lipgloss.JoinHorizontal(lipgloss.Top, m.viewConsoleSidebar(bodyHeight), " ", m.viewConsoleMain(m.width-25, bodyHeight))
	} else {
		nav := m.viewConsoleTopNav()
		body = lipgloss.JoinVertical(lipgloss.Left, nav, m.viewConsoleMain(contentWidth, bodyHeight-lipgloss.Height(nav)-1))
	}
	if m.actionMode != actionNone {
...
func (m Model) viewConsoleMinimum() string {
	box := consoleOverlayBox.Copy().Width(max(30, m.width-4)).Render(
		consoleSection.Render("KUBELOOP OPERATIONS CONSOLE") + "\n\n" +
			"Terminal is too small.\n" + fmt.Sprintf("Current: %dx%d  Required: %dx%d", m.width, m.height, consoleMinWidth, consoleMinHeight) +
...
func (m Model) viewConsoleFooter() string {
	...
	gap := strings.Repeat(" ", max(1, m.width-lipgloss.Width(left)-lipgloss.Width(right)-2))
	return lipgloss.NewStyle().Width(m.width).Foreground(consoleDim).Background(consolePanel).Padding(0, 1).Render(left + gap + right)
}

func (m Model) viewConsoleSidebar(height int) string {
	items := []string{consoleSection.Render("NAVIGATION"), ""}
	...
	items = append(items, "", consoleSubtle.Render("Namespace"), truncateConsole(m.namespace, 18), "", consoleSubtle.Render("Press : for commands"))
	return consoleCard.Copy().Width(20).Height(max(8, height-2)).Render(strings.Join(items, "\n"))
}
...
func (m Model) viewConsoleMain(width, height int) string {
	...
	case tabWorkloads:
		return m.viewConsoleRows("WORKLOADS", m.consoleWorkloadRows(), width, height, "No workloads found in this namespace.")
	case tabServices:
		return m.viewConsoleRows("SERVICES", m.consoleServiceRows(), width, height, "No services found in this namespace.")
	case tabTasks:
...
func (m Model) viewConsoleConnection(width int) string {
	...
	left := consoleCard.Copy().Width(max(24, width/2-3)).Render(
		consoleSection.Render("CONNECTION") + "\n\n" +
			renderConsoleField("State", state) + renderConsoleField("Server", firstNonEmpty(m.activeProfile.DisplayName, m.activeProfile.BaseURL, "Not selected")) +
			renderConsoleField("Namespace", firstNonEmpty(m.namespace, "Not selected")) + renderConsoleField("Mode", strings.ToUpper(string(m.selectedMode))),
	)
	right := consoleDetail.Copy().Width(max(24, width/2-3)).Render(
...
func (m Model) viewConsoleRows(title string, rows []consoleRow, width, height int, empty string) string {
	if len(rows) == 0 {
		...
		return consoleCard.Copy().Width(max(24, width-4)).Height(max(6, height-3)).Render(consoleSection.Render(title) + "\n\n" + m.consoleStateMessage(message))
	}
	state := &m.console.views[m.activeTab]
	...
	if state.focus == focusList {
		listStyle = listStyle.Copy().BorderForeground(consoleTeal)
	}
	list := listStyle.Copy().Width(max(24, listWidth-4)).Height(max(6, height-3)).Render(strings.Join(lines, "\n"))
	if detailWidth == 0 {
		return list
	}
	...
	if state.focus == focusDetail {
		detailStyle = detailStyle.Copy().BorderForeground(consoleTeal)
	}
	detailBody := cropConsoleText(firstNonEmpty(selected.detail, "No additional details."), state.detailOffset, max(3, height-9))
	detail := detailStyle.Copy().Width(max(22, detailWidth-4)).Height(max(6, height-3)).Render(
...
func (m Model) viewConsoleAction(width, height int) string {
	...
	values := "Target: " + firstNonEmpty(m.actionPod, m.actionService, "-") + "\nPort: " + fmt.Sprint(m.actionPort) + "\nCommand: " + m.actionCommand
	controls := ""
	if m.actionMode == actionPortForward {
		portName := ""
		if m.actionPortIndex >= 0 && m.actionPortIndex < len(m.actionPorts) && m.actionPorts[m.actionPortIndex].Name != "" {
			portName = m.actionPorts[m.actionPortIndex].Name + "  "
		}
		remoteValue := fmt.Sprintf("%s%d/%s", portName, m.actionPort, strings.ToUpper(firstNonEmpty(m.actionProtocol, "tcp")))
...
func (m Model) viewConsoleOverlay(width, height int) string {
	...
	switch m.console.overlay {
	case overlayHelp:
		content = consoleSection.Render("KEYBOARD REFERENCE") + "\n\n" + consoleSection.Render("Commands (press :)") + "\n:pods :svc :sessions :conn   Go to view\n:ns      Select namespace\n:servers Manage servers\n:help    This reference\n:q       Quit\n\n" + consoleSection.Render("Keys") + "\n/                 Filter current list\nEsc               Cancel / clear filter\nTab / Shift+Tab   Change view\nj / k             Move or scroll\nPgUp / PgDn       Move one page\n[ / ]             Focus list / details\nEnter             Primary action\nn                 Select namespace\nt                 Filter sessions\ny                 Copy session output\nr                 Refresh\n?                 Close help\nq                 Quit"
	case overlayNamespace:
		matches := m.filteredNamespaces()
		lines := []string{consoleSection.Render("SELECT NAMESPACE"), "", "Search: " + consoleValue.Render(m.console.query+"_"), ""}
	...
	case overlayConfirmDisconnect:
		content = consoleConfirmContent("DISCONNECT?", fmt.Sprintf("%d active session(s) will be interrupted.", m.consoleTaskCount()), "Disconnect")
	case overlayConfirmServiceUninstall:
		content = consoleConfirmContent("UNINSTALL HELPER SERVICE?", "The privileged system Helper Service will be removed.", "Uninstall service")
	case overlayProfiles:
	...
	box := consoleOverlayBox.Copy().Width(minInt(62, width-8)).Render(content)
	return lipgloss.Place(width, height, lipgloss.Center, lipgloss.Center, box)
}

func (m Model) viewConsoleProfiles() string {
	header := lipgloss.NewStyle().Width(m.width).Background(consolePanel).Padding(1, 1).Render(consoleTitle.Render("KUBELOOP") + "  " + consoleSection.Render("SERVERS"))
	footerLeft := consoleSubtle.Render("Enter select   a add   d delete   l login   : cmd   ? help")
	if m.console.inputMode == inputCommand {
		footerLeft = consoleCmdPrompt.Render(":") + consoleCmdText.Render(m.console.inputText+"█") + "   " + consoleSubtle.Render("Enter run   Esc cancel")
	}
	footer := lipgloss.NewStyle().Width(m.width).Foreground(consoleDim).Background(consolePanel).Padding(0, 1).Render(footerLeft)
	...
	if m.loginAdding {
		...
		body = lipgloss.Place(m.width, height, lipgloss.Center, lipgloss.Center, consoleOverlayBox.Copy().Width(minInt(72, m.width-8)).Render(form))
	} else {
		...
		for i, profile := range m.profiles.Profiles {
			...
			line := marker + truncateConsole(name, max(18, m.width/2)) + "  " + truncateConsole(profile.BaseURL, max(18, m.width/2-8))
		...
		body = consoleCard.Copy().Width(m.width - 6).Height(max(6, height-3)).Render(strings.Join(lines, "\n"))
	}
	if m.console.overlay != overlayNone {
...
func (m Model) consoleWorkloadRows() []consoleRow {
	...
	for index, pod := range m.pods {
		...
		if totalContainers == 0 {
			totalContainers = int32(len(pod.Containers))
		}
...
func (m Model) consoleTaskRows() []consoleRow {
	...
	for index, task := range m.portForwards {
		...
		row := consoleRow{title: task.Name, status: "FORWARD", kind: "forward", index: index, meta: task.Address, detail: fmt.Sprintf("State: %s\nNamespace: %s\nKind: %s\nProtocol: %s\nLocal: %s\nRemote: %s:%d", task.State, task.Namespace, task.Kind, task.Protocol, task.Address, task.DialAddress, task.RemotePort)}
		if consoleRowMatchesFilter(row, filter) {
			rows = append(rows, row)
		}
	}
	...
	for index, task := range m.exchanges {
		if m.console.taskFilter == taskFilterAll || m.console.taskFilter == taskFilterTraffic {
			row := trafficConsoleRow("EXCHANGE", "exchange", base+index, task.Service, task.Namespace, task.ClusterIP, task.State, task.Targets)
			if consoleRowMatchesFilter(row, filter) {
				rows = append(rows, row)
			}
		}
	...
	for index, task := range m.previews {
		if m.console.taskFilter == taskFilterAll || m.console.taskFilter == taskFilterTraffic {
			row := trafficConsoleRow("PREVIEW", "preview", base+index, task.Name, task.Namespace, task.ClusterIP, task.State, task.Targets)
			if consoleRowMatchesFilter(row, filter) {
				rows = append(rows, row)
			}
		}
	...
	for index, task := range m.podSSHEndpoints {
		...
		row := consoleRow{title: task.Pod, status: "SSH", kind: "ssh", index: base + index, meta: task.Command, detail: "State: " + task.State + "\nNamespace: " + task.Namespace + "\nPod IP: " + task.PodIP + "\nContainer: " + task.Container + "\nAddress: " + task.Address + "\nCommand: " + task.Command}
		if consoleRowMatchesFilter(row, filter) {
			rows = append(rows, row)
		}
	}
	for index, task := range m.execTasks {
		...
		row := consoleRow{title: task.Pod, status: "EXEC", kind: "exec", index: base + len(m.podSSHEndpoints) + index, meta: task.Command, detail: detail}
		if consoleRowMatchesFilter(row, filter) {
			rows = append(rows, row)
		}
	}
...
func (m Model) consoleTaskCount() int {
	return len(m.portForwards) + len(m.exchanges) + len(m.mirrors) + len(m.previews) + len(m.podSSHEndpoints) + len(m.execTasks)
}
...
func (m Model) consoleTaskTitle() string {
	labels := []string{"ALL", "FORWARD", "TRAFFIC", "SSH", "EXEC"}
...
func (m *Model) copySelectedTaskOutput() tea.Cmd {
	...
	return copySessionToClipboard(row.kind, value)
}
...
func (m Model) viewConsoleProfilesOverlay() string {
	...
	for index, profile := range m.profiles.Profiles {
		...
		lines = append(lines, style.Width(48).Render(marker+truncateConsole(firstNonEmpty(profile.DisplayName, profile.ID), 20)+"  "+truncateConsole(profile.BaseURL, 24)))
	}
	if len(m.profiles.Profiles) == 0 {
...
func consoleConfirmContent(title, message, action string) string {
	return consoleError.Render(title) + "\n\n" + message + "\n\n" + consoleDangerButton.Render(" Enter / y  "+action) + "  " + consoleSubtle.Render("n / Esc cancel")
}
