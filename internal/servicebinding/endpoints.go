...
// without mutating Kubernetes. Callers that persist rollback state must commit
// this snapshot before calling ApplyCapturedServiceIntercept.
func CaptureServiceIntercept(
	ctx context.Context,
	client kubernetes.Interface,
	snapshot *ServiceInterceptSnapshot,
) error {
	return captureServiceIntercept(ctx, client, snapshot)
}

// ApplyCapturedServiceIntercept applies an already-persisted snapshot. On any
// partial mutation it restores that same snapshot before returning an error.
func ApplyCapturedServiceIntercept(
	ctx context.Context,
	client kubernetes.Interface,
	snapshot ServiceInterceptSnapshot,
	interceptID string,
) error {
	return applyCapturedServiceIntercept(ctx, client, snapshot, interceptID)
}
...
func applyServiceIntercept(
	...
	if err := captureServiceIntercept(ctx, client, snapshot); err != nil {
		return err
	}
	return applyCapturedServiceIntercept(ctx, client, *snapshot, interceptID)
}
...
func captureServiceIntercept(
	...
	if err := snapshotEndpointSlices(ctx, client, snapshot); err != nil {
		return err
	}
	return nil
}
...
func applyCapturedServiceIntercept(
	...
	if snapshot.Namespace == "" || snapshot.Service == "" || snapshot.GatewayIP == "" || len(snapshot.Ports) == 0 {
		return fmt.Errorf("captured service intercept snapshot is incomplete")
	}
	service, err := client.CoreV1().Services(snapshot.Namespace).Get(ctx, snapshot.Service, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("get service: %w", err)
	...
	endpointSliceNames, err := currentEndpointSliceNames(ctx, client, snapshot.Namespace, snapshot.Service)
	if err != nil {
		return err
	}
	...
	rollback := func(cause error) error {
		...
		defer cancel()
		if restoreErr := restoreServiceIntercept(rollbackContext, client, snapshot); restoreErr != nil {
			return errors.Join(cause, fmt.Errorf("rollback intercept: %w", restoreErr))
		}
		return cause
	}
	// Some clusters keep the legacy Endpoints object alongside EndpointSlices
	// and run the EndpointSlice mirroring controller. Leaving that object in
	// place lets the controller recreate the original Pod EndpointSlice after
	// we delete it, making traffic alternate between the Gateway and Pods.
	if err := deleteEndpoints(ctx, client, snapshot.Namespace, snapshot.Service); err != nil {
		return rollback(err)
	}
...
func deleteEndpoints(
	...
	err := client.CoreV1().Endpoints(namespace).Delete(ctx, service, metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return fmt.Errorf("delete endpoints %s: %w", service, err)
	}
	return nil
}

func currentEndpointSliceNames(
	ctx context.Context,
	client kubernetes.Interface,
	namespace, service string,
) ([]string, error) {
	list, err := client.DiscoveryV1().EndpointSlices(namespace).List(ctx, metav1.ListOptions{
		LabelSelector: interceptServiceNameLabel + "=" + service,
	})
	if err != nil {
...
func managedEndpointSlice(snapshot ServiceInterceptSnapshot, interceptID string) *discoveryv1.EndpointSlice {
	...
	if address := net.ParseIP(snapshot.GatewayIP); address != nil && address.To4() == nil {
