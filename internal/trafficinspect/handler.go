...
func New(config Config) (*Handler, error) {
	...
	proxy.OnRequest().
		DoFunc(func(request *http.Request, proxyContext *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			emitEvent(config, buildRequestEvent(request, trace))
	...
	proxy.OnResponse().DoFunc(func(response *http.Response, _ *goproxy.ProxyCtx) *http.Response {
		if response != nil && response.Request != nil {
			...
			emitEvent(config, buildResponseEvent(response, trace))
		}
...
// ServeHTTP exposes the same parser as an explicit HTTP proxy side path. It
// is safe to run this handler on a separate loopback listener: the normal
// SOCKS listener and its TCP/UDP forwarding do not depend on it.
func (h *Handler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	if h == nil || h.proxy == nil || request == nil {
		http.Error(writer, "traffic inspection handler is unavailable", http.StatusServiceUnavailable)
		return
	}
	target := request.Host
	...
	if _, _, err := net.SplitHostPort(target); err != nil {
		// Explicit HTTP proxy requests commonly omit the default port.
		target = canonicalAuthority(target, scheme)
	}
	if target == "" {
		http.Error(writer, "traffic inspection target is unavailable", http.StatusBadRequest)
		return
	}
	inspection := &inspectionConnection{
		destination:  canonicalAuthority(target, scheme),
		roundTripper: h.newRoundTripper(target),
	}
	defer inspection.roundTripper.Close()
	ctx := context.WithValue(request.Context(), inspectionConnectionKey{}, inspection)
...
func emitEvent(config Config, event Event) {
