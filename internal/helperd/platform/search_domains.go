...
func windowsSearchDomainMergeScript(desired []string) string {
	...
	b.WriteString("foreach ($root in $roots) { ")
	b.WriteString("if ($value -eq $root -or $value -eq ('svc.'+$root) ")
	b.WriteString("-or $value.EndsWith('.svc.'+$root)) { $isKubernetes=$true; break } }; ")
