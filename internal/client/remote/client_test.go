...
func (refresher *fakeRefresher) Refresh(_ context.Context, _ string, current credentials.Credential) (credentials.Credential, error) {
...
func TestGatewayErrorsDoNotLeakRemoteDetailsOrTokens(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		...
		_, _ = writer.Write([]byte(`{"error":{"code":"FORBIDDEN","message":"operation is not permitted","requestId":"request-1"},"secret":"upstream detail"}`))
	}))
	defer server.Close()
	client, err := New(store, &fakeRefresher{now: now}, Config{HTTPClient: server.Client(), Now: func() time.Time { return now }})
	if err != nil {
...
func TestCapabilitiesCacheIsBoundedByPrincipalNamespaceCredentialAndGatewayVersion(t *testing.T) {
	...
	if _, err := client.Capabilities(context.Background(), serverProfile, "development"); err != nil || capabilityCalls.Load() != 2 {
...
func TestContractHTTPResponseAllowsAdditiveFieldsButRejectsMissingRequiredFields(t *testing.T) {
	...
	missing.Store(true)
	if _, err := client.Version(context.Background(), serverProfile); err == nil || !strings.Contains(err.Error(), "incomplete") {
...
func TestPortForwardTaskLifecycleUsesSessionBoundGatewayAPI(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Method == http.MethodPost {
			if request.Header.Get("Idempotency-Key") != "pf-key" || request.Header.Get("Content-Type") != "application/json" {
				t.Errorf("headers = %#v", request.Header)
			}
			var spec PortForwardSpec
			if err := json.NewDecoder(request.Body).Decode(&spec); err != nil || spec.Name != "api" || spec.RemotePort != 8443 {
...
func TestExchangeTaskLifecycleAndAuthenticatedReverseStream(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Method == http.MethodPost {
			...
			if err := json.NewDecoder(request.Body).Decode(&spec); err != nil || spec.Service != "api" || len(spec.Ports) != 2 {
		...
		_ = json.NewEncoder(writer).Encode(ExchangeTask{
			...
			Ports:     []ExchangePort{{Name: "dns", ServicePort: 53, Protocol: "udp"}, {Name: "http", ServicePort: 80, Protocol: "tcp"}},
	...
	created, err := client.CreateExchange(context.Background(), serverProfile, session, ExchangeSpec{
		Service: "api", Ports: []ExchangePort{{ServicePort: 53, Protocol: "udp"}, {ServicePort: 80, Protocol: "tcp"}},
	}, "exchange-key")
...
func TestMirrorTaskLifecycleAndAuthenticatedShadowStream(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.NewEncoder(writer).Encode(MirrorTask{
			ID: taskID, SessionID: session.ID, Namespace: session.Namespace, State: state,
			Service: "api", ClusterIP: "10.96.0.20",
			Ports:     []MirrorPort{{Name: "dns", ServicePort: 53, Protocol: "udp"}, {Name: "http", ServicePort: 80, Protocol: "tcp"}},
			CreatedAt: now, UpdatedAt: now, ExpiresAt: session.ExpiresAt,
		})
	}))
	defer server.Close()
	...
	created, err := client.CreateMirror(context.Background(), serverProfile, session, MirrorSpec{
		Service: "api", Ports: []MirrorPort{{ServicePort: 53, Protocol: "udp"}, {ServicePort: 80, Protocol: "tcp"}},
	}, "mirror-key")
	if err != nil || created.ID != taskID {
...
func TestPreviewTaskLifecycleAndAuthenticatedReverseStream(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Method == http.MethodPost {
			...
			if err := json.NewDecoder(request.Body).Decode(&spec); err != nil || spec.Name != "local-api" || len(spec.Ports) != 2 {
		...
		_ = json.NewEncoder(writer).Encode(PreviewTask{
			...
			Ports:     []PreviewPort{{Name: "dns", ServicePort: 53, Protocol: "udp"}, {Name: "http", ServicePort: 80, Protocol: "tcp"}},
	...
	created, err := client.CreatePreview(context.Background(), serverProfile, session, PreviewSpec{
		Name: "local-api", Ports: []PreviewPort{{ServicePort: 53, Protocol: "udp"}, {ServicePort: 80, Protocol: "tcp"}},
	}, "preview-key")
...
func TestPreviewTaskRequiresClusterIPOnlyAfterItIsRunning(t *testing.T) {
	...
	task := PreviewTask{
		...
		Name: "local-api", Ports: []PreviewPort{{ServicePort: 80, Protocol: "tcp"}},
		CreatedAt: now, UpdatedAt: now,
	}
...
func TestPodExecTaskOpensAuthenticatedWebSocketStream(t *testing.T) {
	now := time.Now().UTC()
	store := &memoryStore{value: validCredential(now)}
	session := Session{ID: uuid.NewString(), Namespace: "development", State: "active", Generation: 1, CreatedAt: now, UpdatedAt: now, ExpiresAt: now.Add(time.Minute)}
	task := ExecTask{
		ID: uuid.NewString(), SessionID: session.ID, Namespace: session.Namespace, State: "pending",
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.Method == http.MethodPost {
			if request.Header.Get("Idempotency-Key") != "exec-key" || request.Header.Get("Authorization") != "Bearer access-token" {
				t.Errorf("headers = %#v", request.Header)
			}
...
func TestFileTransferTaskUsesAuthenticatedControlAndWebSocketAPIs(t *testing.T) {
	...
	task := FileTransferTask{
		ID: uuid.NewString(), SessionID: session.ID, Namespace: session.Namespace, State: "pending",
		Direction: "download", Kind: "file", Pod: "api-0", Container: "api", RemotePath: "/workspace/data.bin",
		CreatedAt: now, UpdatedAt: now, ExpiresAt: session.ExpiresAt,
	}
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.Header.Get("Authorization") != "Bearer access-token" {
		...
		if strings.HasSuffix(request.URL.Path, "/stream") {
			...
			result, _ := filestream.EncodeResult(filestream.TransferResult{Status: filestream.ResultSucceeded, Transferred: 4})
		...
		if request.Method == http.MethodPost {
			if request.Header.Get("Idempotency-Key") != "file-key" {
				t.Errorf("Idempotency-Key = %q", request.Header.Get("Idempotency-Key"))
			}
			var spec FileTransferSpec
			if err := json.NewDecoder(request.Body).Decode(&spec); err != nil || spec.RemotePath != task.RemotePath || spec.Direction != "download" {
				t.Errorf("spec = %#v err = %v", spec, err)
			}
		}
		_ = json.NewEncoder(writer).Encode(task)
	...
	created, err := client.CreateFileTransferTask(context.Background(), serverProfile, session, FileTransferSpec{
		Direction: "download", Kind: "file", Pod: "api-0", RemotePath: task.RemotePath,
	}, "file-key")
	if err != nil || created.ID != task.ID {
		t.Fatalf("created = %#v err = %v", created, err)
	}
...
func TestFileTransferClientRejectsUnsafeLocalSpecAndGatewayTask(t *testing.T) {
	for _, spec := range []FileTransferSpec{
		{Direction: "upload", Kind: "file", Pod: "api-0", RemotePath: "../escape", Size: 1, Checksum: strings.Repeat("00", 32)},
		{Direction: "upload", Kind: "directory", Pod: "api-0", RemotePath: "/workspace/data", Size: 1, Offset: 1, Checksum: strings.Repeat("00", 32)},
		{Direction: "download", Kind: "file", Pod: "api-0", RemotePath: "/workspace/data", Size: 1},
	} {
	...
	task := FileTransferTask{
		ID: uuid.NewString(), SessionID: session.ID, Namespace: session.Namespace, State: "pending",
		Direction: "download", Kind: "file", Pod: "api-0", Container: "api", RemotePath: "/workspace/../escape",
		CreatedAt: now, UpdatedAt: now, ExpiresAt: now.Add(time.Minute),
	}
...
func TestPodFileClientUsesSessionBoundListAndIdempotentMutationAPIs(t *testing.T) {
	...
	taskID := uuid.NewString()
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.Header.Get("Authorization") != "Bearer access-token" || request.URL.Query().Get("namespace") != session.Namespace {
		...
		case strings.HasSuffix(request.URL.Path, "/list"):
			...
			_ = json.NewEncoder(writer).Encode(PodFileList{
				SessionID: session.ID, Namespace: session.Namespace, Pod: "api-0", Container: "api", Path: "/workspace",
				Items: []PodFileEntry{{Name: "logs", Path: "/workspace/logs", Kind: "directory", Mode: "0755", ModifiedAt: now}},
			})
		case strings.HasSuffix(request.URL.Path, "/create"):
			if request.Header.Get("Idempotency-Key") != "pod-file-key" {
				t.Errorf("Idempotency-Key = %q", request.Header.Get("Idempotency-Key"))
			}
			_ = json.NewEncoder(writer).Encode(PodFileTask{
				ID: taskID, SessionID: session.ID, Namespace: session.Namespace, State: "stopped", Action: "create",
				Pod: "api-0", Container: "api", Path: "/workspace/new", Kind: "directory", Result: PodFileResult{Completed: true},
				CreatedAt: now, UpdatedAt: now, ExpiresAt: session.ExpiresAt,
			})
	...
	serverProfile := profile.Profile{ID: "service-1", BaseURL: server.URL}
	listing, err := client.ListPodFiles(context.Background(), serverProfile, session, PodFileSpec{Pod: "api-0", Path: "/workspace"})
	if err != nil || len(listing.Items) != 1 || listing.Items[0].Name != "logs" {
		t.Fatalf("listing = %#v err = %v", listing, err)
	}
	created, err := client.CreatePodFileOperation(context.Background(), serverProfile, session, "create", PodFileSpec{
		Pod: "api-0", Path: "/workspace/new", Kind: "directory",
	}, "pod-file-key")
...
func TestPodFileClientRejectsUnsafeSpecsAndUnboundResponses(t *testing.T) {
	for action, spec := range map[string]PodFileSpec{
		"list":   {Pod: "api-0", Path: "/workspace/../etc"},
		"create": {Pod: "api-0", Path: "/", Kind: "file"},
		"rename": {Pod: "api-0", Path: "/workspace/a", Destination: "/"},
		"delete": {Pod: "api-0", Path: "relative"},
	} {
	...
	_, err := validatePodFileList(PodFileList{
		SessionID: uuid.NewString(), Namespace: session.Namespace, Pod: "api-0", Container: "api", Path: "/workspace", Items: []PodFileEntry{},
	...
	_, err = validatePodFileTask(PodFileTask{
		ID: uuid.NewString(), SessionID: session.ID, Namespace: session.Namespace, State: "failed", Action: "delete",
		Pod: "api-0", Container: "api", Path: "/workspace/a", Result: PodFileResult{},
		CreatedAt: now, UpdatedAt: now, ExpiresAt: now.Add(time.Minute),
	}, session)
	if err == nil {
