...
func (client *testExchangeClient) OpenExchangeStream(
	ctx context.Context,
	_ profile.Profile,
	_ remote.Session,
	_ remote.ExchangeTask,
...
func TestManagerRelaysTCPAndUDPOnlyToRetainedLocalTargets(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		defer cancel()
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{Type: exchangestream.Ready}); writeErr != nil {
			serverDone <- writeErr
			return
		}
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{
		...
		data, readErr := readTestFrame(ctx, connection)
		...
		halfClose, readErr := readTestFrame(ctx, connection)
		...
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{Type: exchangestream.CloseWrite, StreamID: 1}); writeErr != nil {
			serverDone <- writeErr
			return
		}
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{Type: exchangestream.Close, StreamID: 1}); writeErr != nil {
			serverDone <- writeErr
			return
		}
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{
		...
		datagram, readErr := readTestFrame(ctx, connection)
		...
		if writeErr := writeTestFrame(ctx, connection, exchangestream.Frame{Type: exchangestream.Close, StreamID: 2}); writeErr != nil {
			serverDone <- writeErr
			return
		}
		close(relayVerified)
		stop, readErr := readTestFrame(ctx, connection)
		...
		_ = connection.Close(websocket.StatusNormalClosure, "stopped")
		serverDone <- nil
	}))
	defer server.Close()
	...
	client := &testExchangeClient{endpoint: "ws" + strings.TrimPrefix(server.URL, "http"), task: task}
...
func TestManagerCompensatesWhenExchangeStreamCannotOpen(t *testing.T) {
	now := time.Now().UTC()
	session := remote.Session{ID: uuid.NewString(), Namespace: "development", State: "active", ExpiresAt: now.Add(time.Hour)}
	client := &testExchangeClient{endpoint: "://invalid", task: remote.ExchangeTask{
		ID: uuid.NewString(), SessionID: session.ID, Namespace: session.Namespace, State: "pending",
...
func writeTestFrame(ctx context.Context, connection *websocket.Conn, frame exchangestream.Frame) error {
...
func readTestFrame(ctx context.Context, connection *websocket.Conn) (exchangestream.Frame, error) {
