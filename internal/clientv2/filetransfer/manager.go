...
func (manager *Manager) load() error {
	...
	if err != nil {
		return fmt.Errorf("read V2 file transfer state: %w", err)
	}
	var state persistedState
	if err := json.Unmarshal(contents, &state); err != nil || (state.Version != 1 && state.Version != stateVersion) {
		return errors.New("V2 file transfer state is invalid or unsupported")
	}
	now := manager.now().UTC()
	for _, task := range state.Tasks {
		if err := normalizePersistedTask(&task); err != nil {
			return errors.New("V2 file transfer state contains an invalid Task")
		}
...
func (manager *Manager) persistLocked() error {
	...
	if err != nil {
		return errors.New("encode V2 file transfer state")
	}
