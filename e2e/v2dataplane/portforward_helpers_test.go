...
const portForwardAccessToken = "v2-e2e-port-forward-access"
...
func startPortForwardControlPlane(
	...
	if err := stateStore.Sessions().Create(ctx, storage.Session{
		ID: session.ID, PrincipalID: principalID, DeviceID: deviceID, ClusterID: "v2-e2e",
	...
	sessions, err := sessionapi.New(stateStore, sessionapi.Config{
		ClusterID: "v2-e2e", Networks: staticNetworkDiscoverer{spec: session.NetworkSpec},
	...
	policy, err := authorization.New(authorization.Policy{Rules: []authorization.Rule{{
		ID: "v2-e2e-port-forward", Subjects: []string{principalID}, Namespaces: []string{session.Namespace},
