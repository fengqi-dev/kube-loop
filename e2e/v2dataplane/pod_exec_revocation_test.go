...
func TestRealPodExecStopsWhenTokenFamilyIsRevoked(t *testing.T) {
	...
	if _, err := stateStore.Principals().Upsert(ctx, storage.Principal{
		ID: principalID, Provider: "v2-e2e", ExternalID: "real-pod-user", CreatedAt: now, UpdatedAt: now,
	}); err != nil {
		t.Fatal(err)
	}
	if err := stateStore.TokenFamilies().Create(ctx, storage.TokenFamily{
		ID: familyID, PrincipalID: principalID, DeviceID: "v2-e2e-device",
	...
	if err := stateStore.Sessions().Create(ctx, storage.Session{
		ID: sessionID, PrincipalID: principalID, DeviceID: "v2-e2e-device", ClusterID: "minikube",
	...
	policy, err := authorization.New(authorization.Policy{Rules: []authorization.Rule{{
		ID: "v2-e2e-exec", Subjects: []string{"*"}, Namespaces: []string{harness.EchoNamespace},
	...
	principal := controller.Principal{
		Subject: principalID, DeviceID: "v2-e2e-device", FamilyID: familyID, AccessExpiresAt: expiresAt,
	}
	server, err := controller.NewServer(
		controller.Config{PublicURL: "https://controller.v2-e2e.invalid"}, controller.BuildInfo{},
	...
	createURL := fmt.Sprintf("%s/api/v2/sessions/%s/exec?namespace=%s", httpServer.URL, sessionID, harness.EchoNamespace)
	...
	streamURL := fmt.Sprintf(
		"ws%s/api/v2/sessions/%s/exec/%s/stream?namespace=%s",
