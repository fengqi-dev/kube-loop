...
const copy = {
  en: {
    ...
    quick: "Connect in four steps", quickLead: "Use your existing kubeconfig. KubeLoop handles the local network service and an unprivileged in-cluster Gateway.",
    ...
    architecture: "A narrow, auditable data path", architectureLead: "Only cluster routes enter the tunnel. The Gateway has no Kubernetes credentials and is reached through API Server port-forward.",
    mcp: "A local MCP boundary for agents", mcpLead: "Connect Codex, Claude Code, Cursor, or VS Code to the active authenticated Gateway Session without exposing kubeconfig credentials.",
  },
  zh: {
    nav: ["概览", "快速开始", "流量工作流", "架构", "MCP"],
    eyebrow: "面向本地开发的 Kubernetes 网络工具",
    title: "让本地开发机进入集群网络。",
    intro: "从日常使用的浏览器、IDE、CLI 和 SDK 直接访问 Pod IP、ClusterIP Service 与集群 DNS，无需公网 Gateway，也无需维护大量端口转发。",
    ...
    quick: "四步连接集群", quickLead: "沿用现有 kubeconfig；KubeLoop 负责本地网络服务与集群内无特权 Gateway。",
    workflows: "为任务选择合适的流量路径", workflowsLead: "保留 Service 身份，同时精确决定请求应该去往哪里。",
    architecture: "收敛且可审计的数据路径", architectureLead: "只有集群路由进入隧道。Gateway 不持有 Kubernetes 凭证，并仅通过 API Server port-forward 访问。",
    mcp: "面向 Agent 的本地 MCP 边界", mcpLead: "让 Codex、Claude Code、Cursor 或 VS Code 使用当前已认证的 Gateway Session，而无需暴露 kubeconfig 凭证。",
  },
};
...
function Overview({ t, locale }: { t: typeof copy.en; locale: Locale }) {
  const features = locale === "zh" ? [
    ...
    [ShieldCheck, "最小权限边界", "Gateway 无 ServiceAccount token、hostNetwork、privileged 或 NET_ADMIN。"],
    [Terminal, "Pod SSH 与文件传输", "无需容器运行 sshd，通过 pods/exec 提供 Shell、SFTP 与 SCP。"],
    [Bot, "Agent 工具", "通过本地 MCP 端点安全操作当前活动的 Gateway Session。"],
  ] : [
    ...
    [ShieldCheck, "Minimal trust boundary", "The Gateway has no ServiceAccount token, hostNetwork, privileged mode, or NET_ADMIN."],
    [Terminal, "Pod SSH and files", "Use Shell, SFTP, and SCP over pods/exec without running sshd in the container."],
    [Bot, "Agent tools", "Operate the active authenticated Gateway Session through a local MCP endpoint."],
  ];
...
function TrafficPanel({ locale }: { locale: Locale }) { return <div className="traffic-panel"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><span className="text-xs font-semibold text-white/70">{locale === "zh" ? "活动连接" : "ACTIVE CONNECTION"}</span><span className="flex items-center gap-2 text-xs text-emerald-300"><i className="size-2 rounded-full bg-emerald-400" />Connected</span></div><div className="space-y-5 p-5"><Path from="Local app" to="KubeLoop" detail="TUN + split DNS" /><Path from="KubeLoop" to="API Server" detail="port-forward" /><Path from="Gateway" to="Service" detail="10.96.0.42:443" /><div className="grid grid-cols-3 gap-2 pt-2">{[["12ms", "RTT"], ["24", "Routes"], ["0", "Public ports"]].map(([v,l]) => <div className="rounded-lg border border-white/10 bg-white/5 p-3 text-center" key={l}><strong className="block text-lg text-white">{v}</strong><span className="text-[10px] text-white/50">{l}</span></div>)}</div></div></div>; }
...
function GetStarted({ t, locale }: { t: typeof copy.en; locale: Locale }) { const steps = locale === "zh" ? [["01", "安装 KubeLoop", "下载适用于 macOS、Windows 或 Linux 的发行包。"], ["02", "确认 kubeconfig", "确保开发机可访问 API Server，并拥有有效的 Context。"], ["03", "选择并连接", "选择 Context 与默认 Namespace，然后点击连接。"], ["04", "批准本地 Helper", "首次使用时批准安装；之后它仅管理 KubeLoop 的网络状态。"]] : [["01", "Install KubeLoop", "Download the package for macOS, Windows, or Linux."], ["02", "Confirm kubeconfig", "Make sure the workstation can reach the API Server with a valid Context."], ["03", "Select and connect", "Choose a Context and default Namespace, then click Connect."], ["04", "Approve the local Helper", "Approve installation once; it manages only KubeLoop network state."]]; return <div className="section"><PageIntro eyebrow={locale === "zh" ? "开始" : "START"} title={t.quick} lead={t.quickLead} /><InstallBlock locale={locale} /><div className="mt-10 grid gap-4 md:grid-cols-2">{steps.map(([n,title,body]) => <Card key={n}><CardHeader><Badge className="w-fit">{n}</Badge><CardTitle className="mt-3">{title}</CardTitle><CardDescription>{body}</CardDescription></CardHeader></Card>)}</div><Notice locale={locale} /></div>; }
...
function Architecture({ t, locale }: { t: typeof copy.en; locale: Locale }) { const nodes = [["Local applications", "Browser · IDE · CLI · SDK"], ["Platform TUN + split DNS", "managed sing-box"], ["Local SOCKS Bridge", "session transport"], ["Kubernetes API Server", "port-forward"], ["Unprivileged Gateway", "no Kubernetes credentials"], ["Pods · Services · CoreDNS", "cluster destinations"]]; return <div className="section"><PageIntro eyebrow={locale === "zh" ? "数据路径" : "DATA PATH"} title={t.architecture} lead={t.architectureLead} /><div className="mx-auto max-w-3xl">{nodes.map(([title,detail],i) => <div key={title} className="relative flex gap-4 pb-8 last:pb-0"><div className="flex flex-col items-center"><span className="grid size-10 place-items-center rounded-full border border-primary/25 bg-primary/10 text-sm font-bold text-primary">{i+1}</span>{i < nodes.length - 1 && <span className="w-px flex-1 bg-border" />}</div><Card className="mb-1 flex-1"><CardHeader className="p-4"><CardTitle className="text-sm">{title}</CardTitle><CardDescription className="font-mono text-xs">{detail}</CardDescription></CardHeader></Card></div>)}</div><div className="mt-10 grid gap-4 md:grid-cols-3">{[[ShieldCheck, locale === "zh" ? "无公网入口" : "No public ingress"], [Server, locale === "zh" ? "无特权 Gateway" : "Unprivileged Gateway"], [Route, locale === "zh" ? "仅路由集群流量" : "Cluster routes only"]].map(([Icon,label]) => <div className="flex items-center gap-3 rounded-xl border border-border p-4 text-sm font-semibold" key={String(label)}><Icon className="size-5 text-primary" />{String(label)}</div>)}</div></div>; }

function Mcp({ t, locale }: { t: typeof copy.en; locale: Locale }) { const tools = [["manage_cluster", "Namespaces · Services · Pods"], ["manage_connection", "status · connect · disconnect"], ["manage_traffic", "forward · exchange · mirror · preview"], ["exec_pod_command", "exact argv over Gateway exec"], ["manage_file_transfer", "upload · download · cancel"]]; return <div className="section"><PageIntro eyebrow="MODEL CONTEXT PROTOCOL" title={t.mcp} lead={t.mcpLead} /><div className="grid gap-6 lg:grid-cols-[1fr_.8fr]"><Card><CardHeader><CardTitle>{locale === "zh" ? "可用工具" : "Available tools"}</CardTitle><CardDescription>{locale === "zh" ? "所有集群操作都经过 Gateway SDK、策略与 Kubernetes 授权。" : "Every cluster operation passes through the Gateway SDK, policy, and Kubernetes authorization."}</CardDescription></CardHeader><CardContent className="space-y-2">{tools.map(([name,desc]) => <div key={name} className="flex flex-col justify-between gap-1 rounded-lg border border-border bg-muted/30 p-3 sm:flex-row sm:items-center"><code className="text-xs font-semibold text-primary">{name}</code><span className="text-xs text-muted-foreground">{desc}</span></div>)}</CardContent></Card><Card className="border-primary/20 bg-primary/[.04]"><CardHeader><span className="icon-box"><Bot /></span><CardTitle className="mt-4">{locale === "zh" ? "安全默认值" : "Secure defaults"}</CardTitle></CardHeader><CardContent className="space-y-4 text-sm leading-6 text-muted-foreground">{(locale === "zh" ? ["仅监听 127.0.0.1", "默认生成 Bearer token", "token 存入系统凭证保险库", "仅可使用当前活动 Server Profile", "变更操作必须提供精确 Session 上下文"] : ["Binds only to 127.0.0.1", "Generated Bearer token by default", "Token stays in the OS credential vault", "Uses only the active Server Profile", "Mutations require exact Session context"]).map(x => <p className="flex gap-3" key={x}><Check className="mt-1 size-4 shrink-0 text-primary" />{x}</p>)}</CardContent></Card></div></div>; }

function Cta({ locale }: { locale: Locale }) { return <section className="section pt-0"><div className="cta"><div><Badge className="mb-4 border-white/15 bg-white/10 text-white">MIT · Open source</Badge><h2>{locale === "zh" ? "准备好连接集群了吗？" : "Ready to close the loop?"}</h2><p>{locale === "zh" ? "下载桌面应用，选择 kubeconfig Context，然后开始使用真实的集群地址。" : "Download the desktop app, choose a kubeconfig Context, and start using real cluster addresses."}</p></div><a className={cn(buttonVariants({ size: "lg" }), "bg-white text-emerald-950 hover:bg-white/90")} href={releases}>{locale === "zh" ? "下载最新版本" : "Download latest"}<ArrowRight /></a></div></section>; }
