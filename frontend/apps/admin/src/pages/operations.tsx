...
export function AuditPage() {
  ...
  const exportAudit = async () => { setExporting(true); setError(""); try { const result = await mutation<{ jobId: string }>("/audit/exports", "POST", { limit: 1000, reason: "Export audit records from the management console" }, { idempotent: true }); const blob = await waitForAuditExport(result.jobId); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `kubeloop-audit-${result.jobId}.ndjson`; link.click(); URL.revokeObjectURL(link.href); } catch (cause) { setError((cause as Error).message); } finally { setExporting(false); } };
  const items = state.value?.items ?? [];
