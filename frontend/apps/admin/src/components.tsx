...
export function SecureActionDialog({ open, title, fields, busy, danger, onClose, onConfirm }: { open: boolean; title: string; fields: { name: string; label: string; type?: string; autoComplete?: string; minLength?: number }[]; busy?: boolean; danger?: boolean; onClose: () => void; onConfirm: (values: Record<string, string>) => void }) {
  const { t } = useI18n(); const [values, setValues] = useState<Record<string, string>>({});
  useEffect(() => { if (open) setValues({}); }, [open]);
  const valid = fields.every((field) => (values[field.name] || "").length >= (field.minLength || 1));
