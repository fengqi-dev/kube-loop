...
func main() {
	...
	exitCode := executeOperator(ctx, os.Args[1:], os.Stdout, os.Stderr)
	stop()
	os.Exit(exitCode)
}

func executeOperator(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	return internalcli.Execute(ctx, newOperatorCommand(), args, stdout, stderr)
}

func newOperatorCommand() *cobra.Command {
	return newOperatorCommandWithConfig(newOperatorConfigResolver())
}

func newOperatorCommandWithConfig(config *viper.Viper) *cobra.Command {
	...
	command := &cobra.Command{
		...
		PreRunE: func(*cobra.Command, []string) error {
			...
			if err := config.ReadInConfig(); err != nil {
				return internalcli.Usage(fmt.Errorf("read operator configuration: %w", err))
			}
			return nil
		},
		RunE: func(command *cobra.Command, _ []string) error {
			return runOperator(command.Context(), operatorOptionsFrom(config), zapOptions)
		},
	}
	internalcli.ConfigureRoot(command, "kubeloop-operator")
	internalcli.AddVersionCommand(command, "kubeloop-operator", version)
...
func newOperatorConfigResolver() *viper.Viper { return internalcli.NewViper() }
...
func runOperator(ctx context.Context, options operatorOptions, zapOptions zap.Options) error {
