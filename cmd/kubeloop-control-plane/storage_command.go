...
func runStorageExport(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	config, err := controlplanestorage.ConfigFromEnv()
	if err != nil {
		return err
	}
	raw, metadata, err := controlplanestorage.Export(ctx, config, controlplanestorage.ExportOptions{
		CreatedByVersion: controlPlaneStorageVersion(),
	})
	if err != nil {
		return err
	}
	...
	return writeStorageResult(stdout, struct {
		...
		controlplanestorage.ExportMetadata
...
func runStorageImport(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	result, err := controlplanestorage.Import(ctx, config, raw, controlplanestorage.ImportOptions{
		ConfirmedEmpty: true,
		ImportedBy:     *actor,
	})
	if err != nil {
...
func runStorageBackup(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	result, err := controlplanestorage.BackupSQLite(ctx, config, *output, nil)
	if err != nil {
		return err
	}
	return writeStorageResult(stdout, result)
...
func controlPlaneStorageVersion() string {
	return "kubeloop-control-plane/" + strings.TrimSpace(version) + " commit/" + strings.TrimSpace(commit)
}
...
func readStorageInput(path string) ([]byte, error) {
	...
	if info.Size() > controlplanestorage.MaxExportBytes {
	...
	defer file.Close()
	raw, err := io.ReadAll(io.LimitReader(file, controlplanestorage.MaxExportBytes+1))
	if err != nil {
		return nil, errors.New("read storage import file")
	}
	if len(raw) > controlplanestorage.MaxExportBytes {
		return nil, errors.New("storage import exceeds the size limit")
	}
	return raw, nil
}
...
func printStorageUsage(writer io.Writer) {
	fmt.Fprintln(writer, "Offline Control Plane storage management:")
	fmt.Fprintln(writer, "  kubeloop-control-plane storage export --output FILE [--force]")
	fmt.Fprintln(writer, "  kubeloop-control-plane storage import --input FILE --actor ID --confirm-empty")
	fmt.Fprintln(writer, "  kubeloop-control-plane storage backup --output FILE")
