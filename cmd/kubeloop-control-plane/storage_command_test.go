...
func TestStorageExportAndBackupCommands(t *testing.T) {
	directory := t.TempDir()
	databasePath := filepath.Join(directory, "control-plane.db")
	configureSQLiteCommandTest(t, databasePath)
	store, err := controlplanestorage.Open(context.Background(), controlplanestorage.Config{
		Backend: controlplanestorage.BackendSQLite, SQLitePath: databasePath,
	})
	if err != nil {
		t.Fatal(err)
	}
	principalID := uuid.NewString()
	if _, err := store.Principals().Upsert(context.Background(), controlplanestorage.Principal{
	...
	exportPath := filepath.Join(directory, "exports", "control-plane.json")
	...
	metadata, err := controlplanestorage.ValidateExport(raw)
	if err != nil || metadata.Rows != 1 || metadata.CreatedByVersion != controlPlaneStorageVersion() {
	...
	backupPath := filepath.Join(directory, "backups", "control-plane.db")
	stdout.Reset()
	stderr.Reset()
	...
	backup, err := controlplanestorage.Open(context.Background(), controlplanestorage.Config{
		Backend: controlplanestorage.BackendSQLite, SQLitePath: backupPath,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func configureSQLiteCommandTest(t *testing.T, path string) {
	...
	t.Setenv("KUBELOOP_CONTROL_PLANE_REPLICAS", "1")
