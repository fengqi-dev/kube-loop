...
`internal/operator/` 是 KubeLoop 根项目中的 Kubernetes 控制面组件，不是独立仓库或嵌套 Go module。
它与 Controller、Gateway 共用根目录的 `go.mod`、版本号、CI 和 Helm 发布流程，但以独立进程和
ServiceAccount 部署，以便隔离 Kubernetes 资源协调权限。

根目录的 `PROJECT` 只保存 Kubebuilder 生成器元数据；API 和 TrafficBinding Reconciler 位于
`internal/operator/`，生成清单位于根 `config/`，端到端测试位于 `e2e/operator/`。Go import path
仍属于根 module `github.com/fengqi-dev/kube-loop`。

进程入口统一放在根项目 `cmd/`：`cmd/kubeloop-controller` 运行 HTTP API、认证和任务控制面，
`cmd/kubeloop-operator` 只运行 Kubernetes Reconciler。Operator API 和实现位于
`internal/operator/`，部署与生成配置由根项目统一管理。
...
- `Exchange`：保存原 Service 端点快照后，将流量切换至 Controller relay；删除 CR 时恢复。
- `Mirror`：使用相同的可恢复 Kubernetes 资源生命周期，数据平面的复制语义由 Controller 管理。

Operator 只接受 Controller 写入的可信 relay 地址。客户端凭证、OAuth/OIDC/AD token 和本地桌面
地址不会写入 CRD。

## Controller 与 Operator 的边界

Controller 的 `internal/controller/trafficbindingclient` 是 CR 生命周期客户端。四类任务先写入
...
返回；停止任务时，Controller 删除 CR 并等待对象消失，因此 Operator finalizer 完成恢复或清理
后，Task 才能进入终态。
...
删除、Port Forward 目标的最终校验同样经由 CRD。Controller 的周期回收器会删除已无持久化 Task、
Task 已终止或所有权不匹配的 CR，补上数据库级联删除与 CR 删除之间的崩溃窗口。

Helm 默认同时部署 `kubeloop-controller`、`kubeloop-gateway` 和 `kubeloop-operator`。Controller
...
根项目的应用控制面位于 `internal/controller`；Operator 的 Kubernetes Reconciler 位于
`internal/operator/trafficbinding`，两者职责和包名保持明确分离。根目录的
