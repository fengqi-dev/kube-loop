# ADR 0018: Controller/Data Plane internal protocol
...
Controller currently signs short-lived RelayTickets while Data Plane verifies
...
modes. In mTLS mode, Controller trusts only a configured internal CA and
...
Controller sends it to TokenReview, requires the expected ServiceAccount and a
...
no `relayId` field. Controller derives a stable `relay-<sha256>` ID from that
...
of supported internal versions; Controller selects its highest preference in
that list. During a two-version rolling upgrade, a new peer continues using the
v1 bootstrap envelope and both old-Controller/new-Data-Plane and
new-Controller/old-Data-Plane combinations select v1. A version switch occurs
only after both sides advertise it and accept the registration response. With
no common version, no lease is created.

| Kind | Direction | Purpose |
| --- | --- | --- |
| `RelayRegistration` | Data Plane → Controller | Advertised WSS endpoint, ready/draining state, maximum/current physical connections and logical streams, applied key and revocation generations |
| `RelayRegistrationResult` | Controller → Data Plane | Derived Relay ID, unpredictable lease ID, lease deadline/heartbeat interval, desired state, complete verification key set and revocation summary |
| `RelayHeartbeat` | Data Plane → Controller | Renew exact lease and report current state/capacity plus applied generations |
| `RelayHeartbeatResult` | Controller → Data Plane | New deadline, next heartbeat, desired ready/draining state and latest key/revocation snapshots |
| `SessionAllocation` | Controller internal allocator → Registry | ClusterSession UUID/generation and immutable NetworkSpec hash |
...
- A `draining` report or Controller desired state immediately removes the relay
  from new allocation. Existing assignments and streams remain bound to their
  original Relay ID until they finish, expire or fail; the Controller never
...
Controller responses carry a generation-numbered set of at most eight Ed25519
public keys with `notBefore`/`notAfter`. At least one key must be usable now.
During rotation, Controller publishes the new verification key before signing
...
only in Controller. Data Plane receives public material only.
...
`appliedRevocationGeneration`. Controller must stop assigning/signing for a
...
- Controller can coordinate capacity, drain, key rollout and revocation without
