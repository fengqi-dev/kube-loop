...
Service. The Controller creates a ClusterIP Service and an EndpointSlice that
...
ordinary application resources. A retry, stale Controller, delayed cleanup, or
...
If Service creation succeeds but EndpointSlice creation fails, the Controller
...
local targets. The Controller allocates Gateway listener ports and writes those
...
Controller shutdown, or explicit stop closes listeners and invokes the same
cleanup path.
...
Controller system identity because user credentials may have expired or been
...
- A stale Controller can safely retry cleanup without holding the original user
...
- Operators must grant the Controller ServiceAccount create/get/delete access
...
- **Rely only on process-local defers:** they do not survive Controller loss and
