...
Gateway Pods are replaceable and may be rolled independently from Controller.
...
Kubernetes Service selects a ready Pod; Controller authorizes the selected pool
...
and TUN path, heartbeats Controller to obtain the authoritative newer Session
...
- Kubernetes readiness performs instance reselection while Controller remains
...
- RelayTicket schema v1 is intentionally incompatible with this V2-only branch;
  Controller and Data Plane must roll with overlapping capacity.
