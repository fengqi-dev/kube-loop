...
the cluster client. Controller replicas must also restore every temporary
Service and EndpointSlice mutation after stop, revocation, disconnect or owner
loss.

Redirecting the Service to a Controller listener is necessary to observe both
...
Before mutation, Controller captures the Service selector and all authoritative
EndpointSlice and legacy Endpoints objects. Clusters may maintain both at once;
the Controller deletes both source representations while intercepted so the
...
backend
addresses. Controller derives ready, non-terminating TCP/UDP targets for every
...
For each TCP connection or UDP association, the Controller listener forwards
...
The neutral `mirrorstream` protocol is directional. Controller may send
...
The Controller replica that owns the authenticated WebSocket also owns the
...
listeners and active sockets, restore the snapshot through the Controller
...
- Shadow latency and failure are isolated twice: at the Controller WebSocket
  queue and at each desktop local-target actor.
- Kubernetes credentials, backend discovery, listener allocation and rollback
  remain in Controller. The desktop retains only explicit local host/port
  targets and a typed Gateway SDK.
- Backends observe the Controller Pod as the connection source, and endpoint
