...
V2 initially moved Kubernetes operations from the desktop into the Control Plane.
...
Kubernetes lifecycle object. This made recovery depend on Control Plane workers
...
Its API lives in `api/v1alpha1`; its controllers live in
`internal/controller`. Root-level Kubebuilder metadata and
...
ports and a Control Plane-owned relay IP, but never user tokens, identity-provider
secrets or a desktop destination.

Control Plane follows this ordering:
...
different immutable spec is a conflict. A Control Plane maintenance worker removes
...
- Control Plane keeps authentication, policy and Task/TrafficBinding persistence;
  Gateway owns relay listeners and stream bytes; Operator owns Kubernetes
  resource reconciliation. Gateway drives Task state through authenticated
